    ...
    if type!="100":
        ...
        if length==15:
            ...
            while len(string_to_parse[j:])>0:
                k,total_version,result=parse_packet(string_to_parse[j:],"  "+sub_string,total_version)
                j+=k
                partial_results.append(result)
            i+=value_length
        else:
            for j in range(value_length):
                new_i,total_version,result=parse_packet(string[i:],"  "+sub_string,total_version)
                i+=new_i
                partial_results.append(result)
        if type_dec==0:
            result=0
            ...
            result=min(partial_results)
        elif type_dec==3:
            result=max(partial_results)
        elif type_dec==5:
            result = 1 if partial_results[0]>partial_results[1] else 0
        elif type_dec==6:
            result = 1 if partial_results[0]<partial_results[1] else 0
        elif type_dec==7:
            result = 1 if partial_results[0]==partial_results[1] else 0
        ...
        result=to_dec(literal)
    print(f"{sub_string}Result: {result}")
    return i,total_version,result
...
for char in dec:
    bin+=to_bin(char)
i,version,final_result=parse_packet(bin,"",0)
